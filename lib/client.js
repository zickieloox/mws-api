...
class AmazonMwsClient {
    ...
    constructor(options) {
        ...
        if (!opts.accessKeyId || !opts.secretAccessKey || !opts.merchantId) {
            ...
            console.log(`Missing Amazon client parameter${missing.length > 2 ? 's' : ''}: ${missing.join(', ')}!`);
        }
    ...
    request(req, q, meta) {
        ...
        const body = _.findKey(req.params, {
            type: Type.BODY
        });
        ...
        requestOpts.encoding = 'utf-8' // Update: Zickie Fix wrong encoding for special chracters
        ...
        return this.request(req, q, meta).then((response) => {
            ...
            return parseString(body, {
                ...
                .then((data) => {
                    ...
                    const nextMeta = _.assign({}, meta, {
                        limit: meta.limit - result.length
                    });

                    return _.get(this, req.options.next)({
                            NextToken: nextToken
                        }, nextMeta)
                        .then((nextData) => ({
                ...
                .catch({
                    ...
                    return Promise.delay(backoffDuration)
                        .then(() => this.call(req, q, _.assign({}, meta, {
                            attempt
                        })));
                })
                .catch({
